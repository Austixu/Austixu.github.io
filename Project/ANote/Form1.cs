...
namespace ANote
{
    public partial class MainForm : Form
    {
        ...
        public void textBox1_TextChanged(object sender, EventArgs e)
        {
            ...
            if (tmpc > 1)
            {
                StreamReader reader = new StreamReader(Environment.CurrentDirectory + @"\Temp\" + DateTime.Now.ToString("yyyy-MM-dd-") + (tmpc-1) + ".tmp");
                filec = reader.ReadToEnd();
            }
            if (txtNote.Text == filec)
            {
                tmpc--;
            }
            save = false;
            ...
            StreamWriter tmp = new StreamWriter(Environment.CurrentDirectory + @"\Temp\" + DateTime.Now.ToString("yyyy-MM-dd-") + tmpc + ".tmp");
            tmp.Flush();
            tmp.Write(txtNote.Text);
            tmp.Close();
            tmpc++;
        }
        public void Form1_Load(object sender, EventArgs e)
        {
            Thread tc = new Thread(txtc);
            tc.Start();
            pnlDrag.Width = Screen.PrimaryScreen.Bounds.Width;
            txtNote.Height = this.Height - 54;
            RegisteEvent();
            KeyPreview = true;
            timer1.Start();
            log.Create(filenap);
            ...
            #if true
                log.Write(log.Writetype.Warn, filenap, "Load", "Warning!Mode is Debug!");
            #endif
        } 
        private void txtc()
        {
            if (save == false)
            {
                log.Write(log.Writetype.Warn, filenap, "txtBox", "text changed!");
            }
        }
