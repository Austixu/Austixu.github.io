...
namespace AX
{
    class log
    {
        ...
        public void Write(Writetype writetype,string filepath, string type,string contents)
        {
            ...
            {
                File.AppendAllText(filepath, "[" + DateTime.Now.ToString("HH:mm:ss") + "]" + "[" + type + "/INFO]" + contents+enter);
            } else if(writetype==Writetype.Error)
            {
                File.AppendAllText(filepath, "[" + DateTime.Now.ToString("HH:mm:ss") + "]" + "[" + type + "/ERROR]" + contents + enter);
            } else
            {
                File.AppendAllText(filepath, "[" + DateTime.Now.ToString("HH:mm:ss") + "]" + "[" + type + "/WARN]" + contents+enter); 
            }
        }
    }
}